...
CGFloat rightButtonsLenth = 40;
CGFloat rightButtonsDistance = 75;
CGFloat rightLabelsHeight = 12;
CGFloat rightMargin = 8;
CGFloat distanceBetweenBtnAndLabel = 8;
...
@property (nonatomic, strong) ItemModel *nowItem;
...
- (LikeButton *)loveButton
{
    if (!_loveButton) {
        ...
        CGFloat leftDis = CGRectGetWidth(UIScreen.mainScreen.bounds) - rightButtonsLenth - rightMargin;
        _loveButton = [LikeButton buttonWithType:UIButtonTypeCustom];
        _loveButton.contentMode = UIViewContentModeScaleToFill;
        _loveButton.frame = CGRectMake(leftDis, topDis, rightButtonsLenth, rightButtonsLenth);
...
- (UILabel *)loveLabel
{
    if (!_loveLabel) {
        ...
        _loveLabel.backgroundColor = [UIColor clearColor];
        _loveLabel.text = @"喜 欢";
        _loveLabel.textColor = [UIColor whiteColor];
        _loveLabel.textAlignment = NSTextAlignmentCenter;
        [_loveLabel setFont:[UIFont systemFontOfSize:rightLabelsHeight]];
        [_loveLabel setFrame:CGRectMake(self.loveButton.frame.origin.x, self.loveButton.frame.origin.y + rightButtonsLenth + distanceBetweenBtnAndLabel, rightButtonsLenth, rightLabelsHeight)];
    }
    return _loveLabel;
}
...
- (UIButton *)commentButton
{
    if (!_commentButton) {
        ...
        _commentButton.frame = CGRectMake(self.loveButton.frame.origin.x, self.loveButton.frame.origin.y + rightButtonsDistance, CGRectGetWidth(_loveButton.frame), CGRectGetHeight(_loveButton.frame));
...
- (UILabel *)commentLabel
{
    if (!_commentLabel) {
        ...
        _commentLabel.text = @"看评论";
        _commentLabel.textColor = [UIColor whiteColor];
        _commentLabel.textAlignment = NSTextAlignmentCenter;
        [_commentLabel setFont:[UIFont systemFontOfSize:rightLabelsHeight]];
        [_commentLabel setFrame:CGRectMake(self.commentButton.frame.origin.x, self.commentButton.frame.origin.y + rightButtonsLenth + distanceBetweenBtnAndLabel, rightButtonsLenth, rightLabelsHeight)];
    }
...
- (UIButton *)writeButton
{
    if (!_writeButton) {
        ...
        _writeButton.contentMode = UIViewContentModeScaleToFill;
        _writeButton.frame = CGRectMake(self.loveButton.frame.origin.x, self.loveButton.frame.origin.y - rightButtonsDistance, CGRectGetWidth(_loveButton.frame), CGRectGetHeight(_loveButton.frame));
        _writeButton.backgroundColor = [UIColor clearColor];
        _writeButton.alpha = 0.7;
        
        [_writeButton setImage:[UIImage imageNamed:@"commentButton"] forState:UIControlStateNormal];
...
- (UILabel *)writeLabel
{
    if (!_writeLabel) {
        ...
        _writeLabel.backgroundColor = [UIColor clearColor];
        _writeLabel.text = @"写评论";
        _writeLabel.textColor = [UIColor whiteColor];
        _writeLabel.textAlignment = NSTextAlignmentCenter;
        [_writeLabel setFont:[UIFont systemFontOfSize:rightLabelsHeight]];
        [_writeLabel setFrame:CGRectMake(self.commentButton.frame.origin.x, self.loveButton.frame.origin.y - rightButtonsLenth + distanceBetweenBtnAndLabel, rightButtonsLenth, rightLabelsHeight)];
    }
    return _writeLabel;
}
...
{
    [self.ShowCommentdelegate ShowCommentViewWithItem:self.nowItem];
}

- (void) writeButtonClick
{
    [self.WriteCommentlegate writeCommentToItem:self.nowItem];
}
